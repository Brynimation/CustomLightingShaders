...
Shader "Tutorial/PerVertexLighting"
{
        ...
        */
        Pass
        {
            ...
            Interpolators vert(Attributes input)
            {
                ...
                float3 dirToLight = normalize(_LightPosition - worldSpacePos); 
                float3 normWorldSpace = normalize(mul(unity_ObjectToWorld, input.normal)); //transform vertex normal to world space
                ...
                output.colour = (input.diffuseColour * _AmbientLightIntensity) + (_LightIntensity * input.diffuseColour * clamp(dot(normWorldSpace, dirToLight), 0, 1));
            ...
            half4 frag(Interpolators input) : SV_Target
            {
                ...
                return input.colour * colour *_Tint;
            }
            ENDHLSL
        }
    }
